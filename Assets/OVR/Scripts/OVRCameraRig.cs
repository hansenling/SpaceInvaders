...
#region Unity Messages
	...
	private void UpdateAnchors()
	{
		...
		Vector3 leftTemp = new Vector3 (0, 0, 0);
		float leftTempx = leftEyeAnchor.localPosition.x + OVRManager.capiHmd.GetTrackingState().HeadPose.ThePose.Position.x * 10f;
		float leftTempy = leftEyeAnchor.localPosition.y + OVRManager.capiHmd.GetTrackingState ().HeadPose.ThePose.Position.y * 10f;
			float leftTempz = leftEyeAnchor.localPosition.z + OVRManager.capiHmd.GetTrackingState().HeadPose.ThePose.Position.z* 10f;
		leftEyeAnchor.localPosition = new Vector3(leftTempx, leftTempy, leftTempz);
		Vector3 centerTemp = new Vector3 (0, 0, 0);
		centerEyeAnchor.localPosition = 0.5f * (leftEye.position + rightEye.position);
		float centerTempx = centerEyeAnchor.localPosition.x + OVRManager.capiHmd.GetTrackingState().HeadPose.ThePose.Position.x* 10f;
		float centerTempy = centerEyeAnchor.localPosition.y + OVRManager.capiHmd.GetTrackingState().HeadPose.ThePose.Position.y* 10f;
		float centerTempz = centerEyeAnchor.localPosition.z + OVRManager.capiHmd.GetTrackingState().HeadPose.ThePose.Position.z* 10f;
		centerEyeAnchor.localPosition = new Vector3(centerTempx, centerTempy, centerTempz);
		Vector3 rightTemp = new Vector3 (0, 0, 0);
		rightEyeAnchor.localPosition = rightEye.position;
		float rightTempx = rightEyeAnchor.localPosition.x + OVRManager.capiHmd.GetTrackingState().HeadPose.ThePose.Position.x* 10f;
		float rightTempy = rightEyeAnchor.localPosition.y + OVRManager.capiHmd.GetTrackingState().HeadPose.ThePose.Position.y* 10f;
		float rightTempz = rightEyeAnchor.localPosition.z + OVRManager.capiHmd.GetTrackingState().HeadPose.ThePose.Position.z* 10f;
		rightEyeAnchor.localPosition = new Vector3(rightTempx, rightTempy, rightTempz);
	}
	...
	private Camera ConfigureCamera(OVREye eye)
	{
		...
		// AA is documented to have no effect in deferred, but it cftauses black screens.
